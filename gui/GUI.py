...
class GUI:
    def __init__(self, title="My Application", geometry="300x200", heartModel=None):
        ...
        self.curS1period = tk.IntVar(value=500)
        self.curS2period = tk.IntVar(value=250)
        self.curS1number = tk.IntVar(value=3)
        self.curS2number = tk.IntVar(value=1)
        self.curPulseWidth = tk.IntVar(value=4)
        self.curAmplitude = tk.IntVar(value=20)
        self.curPacePrope = tk.StringVar(value='HRA1')
        

        self.SA_CT_a_Checked = tk.IntVar(value=0)
        self.CA_Checked = tk.IntVar(value=0)
    ...
    def open_pace_panel(self):
        ...
        self.s1Period = tk.Entry(self.pacePanelWindow, textvariable=self.curS1period).grid(row=1, column=1)
        tk.Label(self.pacePanelWindow, text='S2 period').grid(row=0, column=3)
        self.s2Period = tk.Entry(self.pacePanelWindow, textvariable=self.curS2period).grid(row=1, column=3)
        tk.Label(self.pacePanelWindow, text='Pulse width').grid(row=0, column=5)
        self.pulseWidth = tk.Entry(self.pacePanelWindow, textvariable=self.curPulseWidth).grid(row=1, column=5)
        tk.Label(self.pacePanelWindow, text='Amplitude').grid(row=0, column=7)
        self.amplitude = tk.Entry(self.pacePanelWindow, textvariable=self.curAmplitude).grid(row=1, column=7)
        tk.Label(self.pacePanelWindow, text='S1 number').grid(row=3, column=1)
        self.s1Number = tk.Entry(self.pacePanelWindow, textvariable=self.curS1number).grid(row=4, column=1)
        tk.Label(self.pacePanelWindow, text='S2 number').grid(row=3, column=3)
        self.s2Number = tk.Entry(self.pacePanelWindow, textvariable=self.curS2number).grid(row=4, column=3)
        tk.Label(self.pacePanelWindow, text='pace probe').grid(row=3, column=5)
        self.pacePrope = tk.Entry(self.pacePanelWindow, textvariable=self.curPacePrope).grid(row=4, column=5)
    ...
    def open_add_prope(self):
        ...
        self.SA_CT_a = tk.Checkbutton(self.addPropeWindow, text='SA_CT_a', variable=self.SA_CT_a_Checked, onvalue=1, offvalue=0)
        self.SA_CT_a.grid(row=2, column=2)
        self.CA = tk.Checkbutton(self.addPropeWindow, text='CA', variable=self.CA_Checked, onvalue=1, offvalue=0)
